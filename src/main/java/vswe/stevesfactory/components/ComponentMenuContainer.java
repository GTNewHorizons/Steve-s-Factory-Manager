...
public abstract class ComponentMenuContainer extends ComponentMenu {
    ...
    private static final ContainerFilter filter =
            new ContainerFilter(); // this one is static so all of the menus will share the selection
    ...
    public ComponentMenuContainer(FlowComponent parent, ConnectionBlockType validType) {
        ...
        scrollController = new ScrollController<IContainerSelection>(getDefaultSearch()) {
            ...
            protected void onClick(IContainerSelection iContainerSelection, int mX, int mY, int button) {
                if (GuiScreen.isShiftKeyDown() && mX != -1 && mY != -1) {
                    ...
                    setSelectedInventoryAndSync(
                            iContainerSelection.getId(), !selectedInventories.contains(iContainerSelection.getId()));
                }
            }
            ...
            class ToolTip implements IAdvancedTooltip {
                ...
                public ToolTip(GuiManager gui, ConnectionBlock block) {
                    ...
                    for (ForgeDirection direction : ForgeDirection.VALID_DIRECTIONS) {
                        ...
                        String side = Localization.getForgeDirectionLocalization(direction.ordinal())
                                .toString();
                ...
                private void drawBlock(GuiBase gui, int x, int y, int mX, int mY, ForgeDirection direction) {
                    ...
                    gui.drawTexture(
                            x, y, SRC_X, SRC_Y + (CollisionHelper.inBounds(x, y, 16, 16, mX, mY) ? 16 : 0), 16, 16);
                ...
                @SideOnly(Side.CLIENT)
                private boolean drawBlockMouseOver(
                        GuiBase gui, int x, int y, int mX, int mY, ForgeDirection direction) {
            ...
            private void drawMouseOver(
                    GuiManager gui, IContainerSelection iContainerSelection, int x, int y, int mX, int mY) {
        ...
        buttons.add(new PageButton(
                Localization.FILTER_SHORT, Page.MAIN, Localization.FILTER_LONG, Page.FILTER, false, 102, 21));
        buttons.add(new PageButton(
                Localization.MULTI_SHORT, Page.MAIN, Localization.MULTI_LONG, Page.MULTI, false, 111, 21));

        ComponentMenuContainer.Page[] subFilterPages = {
            ComponentMenuContainer.Page.POSITION,
            ComponentMenuContainer.Page.DISTANCE,
            ComponentMenuContainer.Page.SELECTION,
            ComponentMenuContainer.Page.VARIABLE
        };

        for (int i = 0; i < subFilterPages.length; i++) {
            buttons.add(new ComponentMenuContainer.PageButton(
        ...
        buttons.add(
                new Button(
                        Localization.SELECT_INVERT_SHORT, Page.MAIN, Localization.SELECT_INVERT_LONG, false, 102, 60) {
                    @Override
                    void onClick() {
                        for (IContainerSelection iContainerSelection : scrollController.getResult()) {
                            scrollController.onClick(iContainerSelection, -1, -1, 0);
    ...
    void drawContainer(
            GuiManager gui,
            IContainerSelection iContainerSelection,
    ...
    public void onRelease(int mX, int mY, boolean isMenuOpen) {
        filter.currentMenu = this;
        scrollController.onRelease(
                mX, mY); // no need to check we're on the correct menu, this makes sure the holding always stops
    ...
    public boolean onKeyStroke(GuiManager gui, char c, int k) {
        filter.currentMenu = this;
        return currentPage == Page.MAIN
                ? scrollController.onKeyStroke(gui, c, k)
    ...
    public boolean isVariableAllowed(EnumSet<ConnectionBlockType> validTypes, int i) {
        ...
        if (variable.isValid()) {
            EnumSet<ConnectionBlockType> variableValidTypes = ((ComponentMenuContainerTypes)
    ...
    private class PageButton extends Button {
        ...
        private PageButton(
                Localization label, Page page, Localization description, Page targetPage, boolean wide, int x, int y) {
