...
public class GuiRelay extends GuiBase {
    public GuiRelay(final TileEntityRelay relay, InventoryPlayer player) {
        ...
        buttons.add(new Button(Localization.MAKE_EDITOR, BUTTON_X_LEFT, BUTTON_Y_MIDDLE) {
            ...
            public boolean isVisible() {
                return getSelectedPermission() != -1
                        && isOwner(getUserPermission(), true)
    ...
    private static final int PAGE_BUTTON_X_LEFT =
            LIST_POS_X + (LIST_MENU_WIDTH - PAGE_BUTTON_DISTANCE) / 2 - PAGE_BUTTON_SIZE;
    ...
    private int getSelectedPermission() {
        if (selectedPermission < 0
                || selectedPermission >= relay.getPermissions().size()
    ...
    protected void drawGuiContainerBackgroundLayer(float f, int mX, int mY) {
        ...
        if (!relay.doesListRequireOp() || isOp(player, true)) {
            ...
            if (getPageCount() > 1) {
                drawCenteredString(
                        Localization.PAGE.toString() + " " + (currentPage + 1) + " " + Localization.OF.toString() + " "
                                + getPageCount(),
                        LIST_POS_X,
                        PAGE_Y,
                        0.6F,
        ...
        if (info != null) {
            drawString(
                    info.getName(), INFO_BOX_POS_X + INFO_BOX_NAME_X, INFO_BOX_POS_Y + INFO_BOX_NAME_Y, 0.7F, 0x404040);
            for (int i = 0; i < 2; i++) {
    ...
    private int getEndId() {
        return Math.min(
                        getStartId() + PERMISSIONS_PER_PAGE,
                        relay.getPermissions().size())
                - 1;
    }

    private void renderInfoBox(int id, UserPermission permission, int x, int y) {
        int textureId = id == 0
                ? isOwner(permission, false) ? 5 : isOp(permission, false) ? 4 : 3
    ...
    private void drawInfoBoxString(int id, UserPermission permission, int x, int y) {
        String str = id == 0
                ? isOwner(permission, false)
                        ? Localization.PERMISSION_OWNER.toString()
                        : isOp(permission, false)
                                ? Localization.PERMISSION_EDITOR.toString()
                                : Localization.PERMISSION_USER.toString()
                : relay.isCreativeMode()
                        ? permission.isActive()
                                ? Localization.PERMISSION_RESTRICTED.toString()
                                : Localization.PERMISSION_CREATIVE.toString()
                        : permission.isActive()
                                ? Localization.PERMISSION_INVENTORY.toString()
