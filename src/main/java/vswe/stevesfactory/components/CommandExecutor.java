...
public class CommandExecutor {
            ...
            switch (command.getType()) {
                case INPUT:
                    List<SlotInventoryHolder> inputInventory =
                            getInventories(command.getMenus().get(0));
                ...
                case LIQUID_INPUT:
                    List<SlotInventoryHolder> inputTank =
                            getTanks(command.getMenus().get(0));
                    if (inputTank != null) {
                ...
                case REDSTONE_EMITTER:
                    List<SlotInventoryHolder> emitters =
                            getEmitters(command.getMenus().get(0));
                    if (emitters != null) {
                ...
                case REDSTONE_CONDITION:
                    List<SlotInventoryHolder> nodes =
                            getNodes(command.getMenus().get(0));
                    if (nodes != null) {
                ...
                case VARIABLE:
                    List<SlotInventoryHolder> tiles =
                            getTiles(command.getMenus().get(2));
                    if (tiles != null) {
                        updateVariable(
                                tiles,
                ...
                case FOR_EACH:
                    ...
                    return;
                case AUTO_CRAFTING:
                    CraftingBufferElement element = new CraftingBufferElement(
                        ...
                        for (int i = 0; i < parent.getChildrenOutputNodes().size(); i++) {
                            if (command.equals(parent.getChildrenOutputNodes().get(i))) {
                                Connection connection = parent.getConnection(
                                        parent.getConnectionSet().getInputCount() + i);
                                if (connection != null) {
                                    executeCommand(
                ...
                case CAMOUFLAGE:
                    List<SlotInventoryHolder> camouflage =
                            getCamouflage(command.getMenus().get(0));
                    if (camouflage != null) {
                        ComponentMenuCamouflageShape shape = (ComponentMenuCamouflageShape)
                                command.getMenus().get(1);
                        ComponentMenuCamouflageInside inside = (ComponentMenuCamouflageInside)
                                command.getMenus().get(2);
                        ComponentMenuCamouflageSides sides = (ComponentMenuCamouflageSides)
                                command.getMenus().get(3);
                            ...
                            for (SlotInventoryHolder slotInventoryHolder : camouflage) {
                                ...
                                for (int i = 0; i < ForgeDirection.VALID_DIRECTIONS.length; i++) {
                                    if (sides.isSideRequired(i)) {
                                        slotInventoryHolder
                                                .getCamouflage()
                                                .setItem(itemStack, i, inside.getCurrentType());
                                    }
                                }
                            }
                        }
                    ...
                    if (val == i) {
                        ...
                        for (int selected : selection) {
                            addContainer(
                                    ...
                                    menuContainer,
                                    type,
                                    ((ComponentMenuContainerTypes) variable.getDeclaration()
                                                    .getMenus()
                                                    .get(1))
                                            .getValidTypes());
                        }
    ...
    private static boolean containsTe(List<SlotInventoryHolder> lst, TileEntity te) {
        for (SlotInventoryHolder slotInventoryHolder : lst) {
            if (slotInventoryHolder.getTile().xCoord == te.xCoord
    ...
    private void addItemToBuffer(
        ...
        if ((menuItem.useWhiteList() == (setting != null)) || (setting != null && setting.isLimitedByAmount())) {
            ...
            if (!added) {
                ItemBufferElement itemBufferElement =
                        new ItemBufferElement(owner, setting, inventory, menuItem.useWhiteList(), target);
                    ...
                    for (int side : slot.getSides()) {
                        ...
                        for (FluidTankInfo fluidTankInfo : currentTankInfos) {
                            ...
                            for (FluidTankInfo tankInfo : tankInfos) {
                                if (FluidStack.areFluidStackTagsEqual(tankInfo.fluid, fluidTankInfo.fluid)
                                        && tankInfo.capacity == fluidTankInfo.capacity) {
                                    alreadyUsed = true;
                                }
                            }
    ...
    private void addLiquidToBuffer(
        ...
        if ((menuItem.useWhiteList() == (setting != null)) || (setting != null && setting.isLimitedByAmount())) {
            ...
            if (!added) {
                LiquidBufferElement itemBufferElement =
    ...
    private void insertItems(ComponentMenu componentMenu, List<SlotInventoryHolder> inventories) {
        ...
        for (SlotInventoryHolder inventoryHolder : inventories) {
            ...
            for (CraftingBufferElement craftingBufferElement : craftingBufferHigh) {
                insertItemsFromInputBufferElement(
    ...
    private void insertItemsFromInputBufferElement(
        ...
        while ((subElement = itemBufferElement.getSubElement()) != null) {
            ...
            if (outputItemCounter == null) {
                outputItemCounter =
                        new OutputItemCounter(itemBuffer, inventories, inventory, setting, menuItem.useWhiteList());
                ...
                while (liquidIterator.hasNext()) {
                    ...
                    if (outputLiquidCounter == null) {
                        outputLiquidCounter = new OutputLiquidCounter(
                                liquidBuffer, tanks, tankHolder, setting, menuItem.useWhiteList());
            ...
            for (int i = 0; i < connections.length; i++) {
                ...
                if (!connectionOption.isInput() && connection != null) {
                    ...
                    CommandExecutor newExecutor = new CommandExecutor(
                            ...
                            new ArrayList<CraftingBufferElement>(craftingBufferLow),
                            liquidBufferSplit,
                            usedCommandCopy);
                    newExecutor.executeCommand(
                            manager.getFlowItems().get(connection.getComponentId()), connection.getConnectionId());
                    usedId++;
                }
            }
    ...
    private void updateVariable(
        ...
        if (variable.isValid()) {
            ...
            List<ConnectionBlock> inventories = manager.getConnectedInventories();
            EnumSet<ConnectionBlockType> validTypes = ((ComponentMenuContainerTypes)
            ...
            for (int id : idList) {
                if (remove) {
                    variable.remove(id);
                        ...
                        && id < inventories.size()
                        && inventories.get(id).isOfAnyType(validTypes)) {
                    variable.add(id);
                }
            }
        }
