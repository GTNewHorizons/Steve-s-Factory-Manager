...
public class TileEntityBreaker extends TileEntityClusterElement implements IInventory, IPacketBlock {
    ...
    private List<ItemStack> getInventory() {
        if (inventory == null) {
            ForgeDirection direction = ForgeDirection.VALID_DIRECTIONS[getBlockMetadata()
    ...
    private List<ItemStack> placeItem(ItemStack itemstack) {
        ...
        if (itemstack != null && itemstack.getItem() != null && itemstack.stackSize > 0) {
            ForgeDirection side = ForgeDirection.VALID_DIRECTIONS[getBlockMetadata()
                    % ForgeDirection.VALID_DIRECTIONS.length];
            ...
            EntityPlayerMP player = FakePlayerFactory
                    .get((WorldServer) worldObj, new GameProfile(FAKE_PLAYER_ID, FAKE_PLAYER_NAME));
            int rotationSide = ROTATION_SIDE_MAPPING[direction.ordinal()];

            player.prevRotationPitch = player.rotationYaw = rotationSide * 90;
            player.prevRotationYaw = player.rotationPitch = direction == ForgeDirection.UP ? 90
                    : direction == ForgeDirection.DOWN ? -90 : 0;
            ...
            if (itemStack != null) {
                ...
                if (!broken) {
                    for (int i = 0; i < inventoryCache.size(); i++) {
                        ...
                        if (copyStack != null && newStack.isItemEqual(copyStack)
    ...
    public void markDirty() {
        ...
        if (inventory != null && !broken) {
            ...
            for (int i = 0; i < inventory.size(); i++) {
                ...
                if (itemStackCopy != null && (itemStack == null
