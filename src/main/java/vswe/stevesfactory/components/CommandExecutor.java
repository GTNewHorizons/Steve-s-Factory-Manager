...
public class CommandExecutor {
    ...
    private CommandExecutor(TileEntityManager manager, List<ItemBufferElement> itemBufferSplit,
            List<CraftingBufferElement> craftingBufferHighSplit, List<CraftingBufferElement> craftingBufferLowSplit,
            List<LiquidBufferElement> liquidBufferSplit, List<Integer> usedCommandCopy) {
        this.manager = manager;
    ...
    private static boolean containsTe(List<SlotInventoryHolder> lst, TileEntity te) {
        for (SlotInventoryHolder slotInventoryHolder : lst) {
            if (slotInventoryHolder.getTile().xCoord == te.xCoord && slotInventoryHolder.getTile().yCoord == te.yCoord
    ...
    private void addItemToBuffer(ComponentMenuStuff menuItem, SlotInventoryHolder inventory, Setting setting,
        ...
        if ((menuItem.useWhiteList() == (setting != null)) || (setting != null && setting.isLimitedByAmount())) {
            ...
            if (!added) {
                ItemBufferElement itemBufferElement = new ItemBufferElement(
                        owner,
                        setting,
                        inventory,
            ...
            if (tank.getTank() instanceof TileEntityCreative) {
                ...
                for (SlotSideTarget slot : tank.getValidSlots().values()) {
                    ...
                    for (int side : slot.getSides()) {
                        ...
                        for (FluidTankInfo fluidTankInfo : tank.getTank()
                                .getTankInfo(ForgeDirection.VALID_DIRECTIONS[side])) {
                            if (fluidTankInfo != null) {
    ...
    private void addLiquidToBuffer(ComponentMenuStuff menuItem, SlotInventoryHolder tank, Setting setting,
        ...
        if ((menuItem.useWhiteList() == (setting != null)) || (setting != null && setting.isLimitedByAmount())) {
            ...
            if (!added) {
                LiquidBufferElement itemBufferElement = new LiquidBufferElement(
                        owner,
                        setting,
                        tank,
                        menuItem.useWhiteList(),
    ...
    private void insertItemsFromInputBufferElement(ComponentMenuStuff menuItem, List<SlotInventoryHolder> inventories,
            List<OutputItemCounter> outputCounters, SlotInventoryHolder inventoryHolder,
        ...
        while ((subElement = itemBufferElement.getSubElement()) != null) {
            ...
            if (outputItemCounter == null) {
                outputItemCounter = new OutputItemCounter(
                        itemBuffer,
                        inventories,
                        inventory,
                        setting,
            ...
            for (SlotSideTarget slot : inventoryHolder.getValidSlots().values()) {
                ...
                if (newItem
                        || (itemInSlot.isItemEqual(itemStack) && ItemStack.areItemStackTagsEqual(itemStack, itemInSlot)
